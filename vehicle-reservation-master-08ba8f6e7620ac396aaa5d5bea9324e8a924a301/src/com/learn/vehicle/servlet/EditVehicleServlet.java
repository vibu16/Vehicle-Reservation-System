...
public class EditVehicleServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			Date nextAvailableDate = DateUtil.convertToDate(request.getParameter("nextavailable"));
			long price = Long.parseLong(request.getParameter("vehiprice")); 
			
			Vehicle vehicle=new Vehicle(vehicleNumber, name, color, branch, vehicleType, insuranceExpiryDate, lastServicedDate, serviceDueDate, nextAvailableDate, seatingCapacity, yearOfManufacture, price);
			VehicleDao vehicleDao = new VehicleDaoImpl();
			vehicleDao.modifyVehice(vehicle);
			request.setAttribute("msg", "Vehicle details saved successfully");
